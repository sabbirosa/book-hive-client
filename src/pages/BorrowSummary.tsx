...
import { useGetBorrowStatisticsQuery, useGetBorrowSummaryQuery } from "@/redux/api/borrowsApi";
import type { BorrowSummary } from "@/types";
import { BarChart3, BookOpen, Clock, TrendingUp } from "lucide-react";

export default function BorrowSummary() {
  const { data: summaryData, isLoading: isLoadingSummary, error: summaryError } = useGetBorrowSummaryQuery();
  const { data: statsData, isLoading: isLoadingStats, error: statsError } = useGetBorrowStatisticsQuery();

  if (isLoadingSummary || isLoadingStats) {
    return (
      <div className="container mx-auto py-8">
        <div className="text-center">Loading borrow summary...</div>
      </div>
    );
  }
      ...
      <div className="container mx-auto py-8 px-4">
        <div className="text-center">
          ...
          <p className="text-muted-foreground mb-4">
            There was an error loading the borrow summary. Please try again later.
          </p>
      ...
      {stats && (
        <div className="grid md:grid-cols-4 gap-4 mb-8">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{stats.totalQuantityBorrowed}</div>
              <p className="text-xs text-muted-foreground">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                {stats.mostBorrowedBook?.totalQuantityBorrowed || 0} times borrowed
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <CardContent>
          {summary.length === 0 ? (
            <div className="text-center py-8">
              ...
              <h3 className="text-lg font-semibold mb-2">No Borrowing Activity</h3>
              <p className="text-muted-foreground">
                ...
                <TableBody>
                  {summary.map((item: BorrowSummary) => (
                    <TableRow key={item._id}>
                      ...
                      <TableCell>{item.book?.author || "Unknown Author"}</TableCell>
                      <TableCell>
                        <Badge variant="outline">{item.book?.genre || "Unknown"}</Badge>
                      </TableCell>
                      <TableCell className="font-mono">{item.book?.isbn || "N/A"}</TableCell>
